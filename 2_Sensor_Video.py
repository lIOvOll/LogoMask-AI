...
def encode_ke_h264(path_input, path_output):
    """
    Re-encode video hasil OpenCV (codec mp4v) menjadi H.264 (avc1)
    agar bisa diputar di browser (st.video) & Streamlit Cloud.
    Codec mp4v dari cv2.VideoWriter TIDAK didukung oleh browser.
    """
    if shutil.which("ffmpeg") is None:
        # ffmpeg tidak tersedia di environment -> kembalikan file asli
        return False
    ...
    perintah = [
        ...
        "-i", path_input,
        "-vcodec", "libx264",
        "-pix_fmt", "yuv420p",   # wajib untuk kompatibilitas browser
        "-movflags", "+faststart",
        "-preset", "fast",
        "-crf", "23",
        path_output
    ...
    return hasil.returncode == 0 and os.path.exists(path_output)
...
# Processing Handler
    ...
    with st.status("⏳ Memproses pipeline AI...", expanded=True) as status:
        ...
        # Re-encode ke H.264 supaya video bisa diputar di browser (st.video / Streamlit Cloud)
        teks_status.text("Mengonversi video ke format yang kompatibel dengan browser...")
        temp_output_final = tempfile.NamedTemporaryFile(delete=False, suffix='.mp4')
        temp_output_final.close()

        berhasil_encode = encode_ke_h264(temp_output_raw.name, temp_output_final.name)
        path_video_final = temp_output_final.name if berhasil_encode else temp_output_raw.name

        if not berhasil_encode:
            st.warning(
                "⚠️ ffmpeg tidak ditemukan di server, video mungkin tidak bisa di-preview "
                "di browser (tetap bisa diunduh). Tambahkan 'ffmpeg' pada file packages.txt "
                "agar preview berfungsi di Streamlit Cloud.",
                icon="⚠️"
            )

        status.update(label="Selesai", state="complete", expanded=False)

    with open(path_video_final, 'rb') as f:
        st.session_state.video_hasil = f.read()

    os.unlink(temp_input.name)
    os.unlink(temp_output_raw.name)
    if berhasil_encode and os.path.exists(temp_output_final.name):
...
# Output View
    ...
    st.info("💡 Video berhasil diproses sepenuhnya! Silakan tonton preview atau unduh file di bawah.", icon="ℹ️")
    ...
    with col1:
        st.download_button("Download Video", st.session_state.video_hasil, "output_video.mp4", "video/mp4", use_container_width=True, type="primary")
    with col2:
